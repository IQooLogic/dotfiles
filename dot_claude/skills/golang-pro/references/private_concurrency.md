...
Stage-based processing pipelines chain transformations through channels,
with each stage running in its own goroutine:
...
- Context cancellation propagates through all stages via select.
- Stages compose naturally: chain more stages by reading from the previous stage's output.
- Combine with fan-out/fan-in (section 5) to parallelize individual stages.
